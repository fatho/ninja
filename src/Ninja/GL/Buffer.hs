...
class BufferData a where
  ...
  withRawData :: a -> (GLsizeiptr -> Ptr () -> IO ()) -> IO ()
  -- | Provide a buffer of the given size for the data to be read into.
  fromRawData :: GLsizeiptr -> (Ptr () -> IO ()) -> IO a

instance Storable a => BufferData (VS.Vector a) where
  withRawData vs f = VS.unsafeWith vs $ \p -> f (fromIntegral $ VS.length vs * sizeOf (undefined :: a)) (castPtr p)
...
instance Storable a => BufferData [a] where
  withRawData vs f = allocaArray l $ \p -> pokeArray p vs >> f (fromIntegral $ l * sizeOf (undefined :: a)) (castPtr p)
    where l = length vs
  fromRawData size f = allocaArray l $ \p -> f (castPtr p) >> peekArray l p
...
bufferData (BufferTarget _ t) (BufferUsage usage) dat = liftIO $ withRawData dat $ \s p -> glBufferData t s p usage

-- | Modifies a buffer, see 'glBufferSubData'.
bufferSubData :: (MonadIO m, BufferData a) => BufferTarget -> GLintptr -> a -> m ()
bufferSubData (BufferTarget _ t) off dat = liftIO $ withRawData dat $ \s p -> glBufferSubData t off s p

-- | Modifies a buffer, see 'glBufferSubData'.
getBufferSubData :: (MonadIO m, BufferData a) => BufferTarget -> GLintptr -> GLsizeiptr -> m a
getBufferSubData (BufferTarget _ t) off size = liftIO $ fromRawData size $ \p -> glGetBufferSubData t off size p
